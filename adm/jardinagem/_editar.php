...
if ($passo == 1) {
    ...
    mysql_query("INSERT INTO jardinagem (jardim, descricao, problema, solucao, status) VALUES ('$jardim', '$descricao', '$problema', '$solucao', '$status')");
    ...
    $ext1 = $ext1[1];
    $b1 = "foto_jardinagem_G_(".$id.").".$ext1;
    move_uploaded_file($a1,"../../fotos_jardinagem/G/".$b1);

    preg_match("/.(gif|bmp|png|jpg|jpeg){1}$/i", $fotoPequena["name"], $ext2);
    $a2 = $fotoPequena['tmp_name'];
    $ext2 = $ext2[1];
    $b2 = "foto_jardinagem_P_(".$id.").".$ext2;
    move_uploaded_file($a2,"../../fotos_jardinagem/P/".$b2);

    mysql_query("UPDATE jardinagem SET foto_grande = '$b1', foto_pequena = '$b2' WHERE id = '$id'");
    ...
    if ($fotoGrande["name"] != null || $fotoPequena["name"] != null) {
        ...
        $a1 = $fotoGrande['tmp_name'];
        $ext1 = $ext1[1];
        $b1 = "foto_jardinagem_G_(".$idResult.").".$ext1;
        move_uploaded_file($a1,"../../fotos_jardinagem/G/".$b1);

        preg_match("/.(gif|bmp|png|jpg|jpeg){1}$/i", $fotoPequena["name"], $ext2);
        $a2 = $fotoPequena['tmp_name'];
        $ext2 = $ext2[1];
        $b2 = "foto_jardinagem_P_(".$idResult.").".$ext2;
        move_uploaded_file($a2,"../../fotos_jardinagem/P/".$b2);
    ...
    mysql_query("UPDATE jardinagem SET jardim = '$jardim', status = '$status' $sql WHERE id = '$idResult'");
