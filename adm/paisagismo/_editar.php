...
if ($passo == 1) {
    ...
    if ($projeto == null) { header("location:cadastrar.php?error=new"); exit(); }
    if ($fotoGrande["name"] == null || $fotoPequena["name"] == null) { header("location:cadastrar.php?error=new_foto"); exit(); }

    mysql_query("INSERT INTO paisagismo (projeto, descricao, problema, solucao, status) VALUES ('$projeto', '$descricao', '$problema', '$solucao', '$status')");
    ...
    if ($projeto == null) { header("location:cadastrar.php?error=new&passo=1&id=$idResult"); exit(); }
    ...
    mysql_query("UPDATE paisagismo SET projeto = '$projeto', status = '$status' $sql WHERE id = '$idResult'");
