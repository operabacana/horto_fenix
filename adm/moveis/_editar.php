...
if ($passo == 1) {
    ...
    $tipo = $_POST["tipo"];
    if(count($tipo) > 1){ $tipo = 3; }else{ $tipo = $tipo[0]; }
    $status = $_POST["status"];
    $fotoGrande = $_FILES["fotoGrande"];
    $fotoPequena = $_FILES["fotoPequena"];

    if ($produto == null) { header("location:cadastrar.php?error=new"); exit(); }
    if ($fotoGrande["name"] == null || $fotoPequena["name"] == null) { header("location:cadastrar.php?error=new_foto"); exit(); }

    mysql_query("INSERT INTO moveis (produto, descricao, problema, solucao, tipo, status) VALUES ('$produto', '$descricao', '$problema', '$solucao', '$tipo', '$status')");
    ...
    $ext1 = $ext1[1];
    $b1 = "foto_Produto_G_(".$id.").".$ext1;
    move_uploaded_file($a1,"../../fotos_moveis/G/".$b1);

    preg_match("/.(gif|bmp|png|jpg|jpeg){1}$/i", $fotoPequena["name"], $ext2);
    $a2 = $fotoPequena['tmp_name'];
    $ext2 = $ext2[1];
    $b2 = "foto_Produto_P_(".$id.").".$ext2;
    move_uploaded_file($a2,"../../fotos_moveis/P/".$b2);

    mysql_query("UPDATE moveis SET foto_grande = '$b1', foto_pequena = '$b2' WHERE id = '$id'");
    ...
    if ($produto == null) { header("location:cadastrar.php?error=new&passo=1&id=$idResult"); exit(); }
    ...
    if ($fotoGrande["name"] != null || $fotoPequena["name"] != null) {
        ...
        $a1 = $fotoGrande['tmp_name'];
        $ext1 = $ext1[1];
        $b1 = "foto_Produto_G_(".$idResult.").".$ext1;
        move_uploaded_file($a1,"../../fotos_moveis/G/".$b1);

        preg_match("/.(gif|bmp|png|jpg|jpeg){1}$/i", $fotoPequena["name"], $ext2);
        $a2 = $fotoPequena['tmp_name'];
        $ext2 = $ext2[1];
        $b2 = "foto_Produto_P_(".$idResult.").".$ext2;
        move_uploaded_file($a2,"../../fotos_moveis/P/".$b2);
    ...
    mysql_query("UPDATE moveis SET produto = '$produto', status = '$status' $sql WHERE id = '$idResult'");
